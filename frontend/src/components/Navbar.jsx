        ...
        width:
            ...
            height:
              isMobile
                ...
                ? 84
                : 90, 
            objectFit:"contain", position:"relative",top:isMobile ? 0 : -4, right:isMobile ? 0 : 0 }} />
        <span className="clash" style={{
  fontSize:isMobile ? 22 : isTablet ? 28 : 30,
  fontWeight:700,
  color:"white",
  letterSpacing:isMobile ? .5 : 2,
  whiteSpace:"nowrap"
}}>SkillExchange</span>
      </div>
...
border:"1px solid rgba(255,255,255,.06)",borderRadius:40,padding:"6px 6px"}}>
        {NAV_LINKS.map(n=>(
          <a key={n.label} href={n.href} data-tip={n.label.toUpperCase()}
            style={{padding:isTablet ? "12px 8px" : "12px 8px",borderRadius:30,fontSize:isTablet ? 10 : 11,letterSpacing:1,textTransform:"uppercase",color:"rgba(255,255,255,.36)",textDecoration:"none",transition:"all .3s"}}
            onMouseEnter={e=>{e.target.style.color="white";e.target.style.background="rgba(255,255,255,.05)";}}
            onMouseLeave={e=>{e.target.style.color="rgba(255,255,255,.36)";e.target.style.background="transparent";}}>
            {n.label}
          </a>
        ))}
      </div>
    ...
    {menuOpen ? "✕" : "☰"}
      ...
      {isAuth && user && !isMobile ? (
        <div style={{display:"flex",alignItems:"center",gap:isMobile ? 8 : 12}}>
          ...
          {/* + List Skill */}
          <MagneticButton onClick={onAddSkill}
            style={{border:"1px solid rgba(255,200,50,.3)",color:"#ffc832",borderRadius:30,padding:
                    isMobile
                      ? "12px 14px"
                      : "16px 20px",
...
>
  ...
  <span style={{fontSize:25,color:"rgba(255,255,255,.3)"}}>▾</span>
</div>

            {/* Dropdown */}
            <div style={{
              position:"absolute", top:"calc(100%)", right:7,
              width:200, background:"#0d0e16",
              border:"1px solid rgba(255,255,255,.08)", borderRadius:16,
              overflow:"hidden", boxShadow:"0 20px 50px rgba(0,0,0,.6)",
              opacity: hovering ? 1 : 0,
              transform: hovering ? "translateY(0) scale(1)" : "translateY(-8px) scale(.97)",
              pointerEvents: hovering ? "all" : "none",
              transition:"opacity .25s, transform .25s",
              zIndex:300,
              padding:"10px"
            }}>
              <div style={{height:1,background:"linear-gradient(90deg,transparent,#ffc832,transparent)"}}/>
              {[
                ...
                <div key={i}
                  onClick={item.action}
                  style={{
                    padding:"15px 25px", fontSize:16, color:"rgba(255,255,255,.6)",
                    cursor:"pointer", transition:"all .2s",
                    borderBottom: i < 2 ? "1px solid rgba(255,255,255,.04)" : "none",
                  }}
                  onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,200,50,.08)";e.currentTarget.style.color="white";}}
          ...
          <MagneticButton
            ...
            style={{border:"1px solid rgba(255,255,255,.2)",color:"rgba(255,255,255,.5)",borderRadius:30,padding:"17px 20px",fontSize:12,letterSpacing:3,textTransform:"uppercase",fontWeight:600,background:"transparent"}}>
            Sign Out
          </MagneticButton>
